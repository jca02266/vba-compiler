...
本エンジンの**拡張機能**として、`.bas` モジュール内に `Class` キーワードを使用してクラスを定義可能にしています。これにより、テストコード内で複数のクラス定義を1つのファイルにまとめることができ、ユニットテスト環境での利便性が向上します。
...
このプロジェクトには `src/compiler/mock/MockWorksheet.ts` として `MockApplication` と `MockWorksheet` が実装済みです。`Sheets(...)` のような Excel 依存を分離しきれない場合にモックを使います。
...
| ドキュメント | 内容 |
|------------|------|
| [TYPE_SYSTEM_SPEC.md](docs/TYPE_SYSTEM_SPEC.md) | VBA 型システムの仕様とこのプロジェクトでの実装方針 |
