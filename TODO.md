...
- [ ] **`Empty` / `Null` / `Nothing` / `Missing` の演算子挙動**: `Null + 5 = Null`、`Empty + 5 = 5` などの伝播規則
...
- [ ] **`On Error Resume Next` 下の暗黙の `Err.Clear`**: 正常文実行時のクリア（または非クリア）タイミング
- [ ] **`Resume` の対象決定**: エラー発生点・Resume Next・Resume <label> の正確な制御フロー
- [ ] **エラーハンドラ内での再帰的なエラー発生**: スタックフレームのリセット規則
- [ ] **`Erl` 関数**: エラー発生行番号の取得（要 lexer の行番号付与）

### プロシージャ呼び出しの細部

- [ ] **ByRef での文字列・配列・オブジェクトの参照保持**: 内部での再代入が呼び出し元にも反映されるか
- [ ] **ParamArray の境界ケース**: 0 個渡し、配列を 1 つだけ渡したときの展開規則
...
- [ ] **`Like` 演算子の文字クラス `[a-z]` 詳細**: 範囲外文字・特殊エスケープの扱い
- [ ] **`Mid$` 代入文の長さ規則**: 元文字列長を超える代入の挙動
- [ ] **`StrComp` の Option Compare 影響**: モジュールレベル設定の伝播
- [ ] **ANSI/Unicode 切り替え**: `StrConv(..., vbFromUnicode)` 等の挙動

### Late Binding 周辺

- [x] **`CreateObject(progId)` 経由のオブジェクト呼び出し**: 動的ディスパッチ
- [ ] **存在しないメソッド呼び出しのエラー詳細**: VBA エラー番号 438 を厳密に再現
- [ ] **`IDispatch` 風のプロパティ get/set 自動振り分け**: 引数の有無でメソッド/プロパティ自動判定
