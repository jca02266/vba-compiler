...
| 状態 | 優先度 | 機能 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P0 | Call Statement | §5.4.2.1 | 未テスト |
| ✅ | P1 | Named Parameters | §5.6.3.1.2 | 未テスト |
| ✅ | P0 | While Statement (`While...Wend`) | §5.4.2.2 | `while-wend.test.ts` |
| ✅ | P0 | For Statement (`For...Next`、Step対応) | §5.4.2.3 | 未テスト |
| ✅ | P0 | For Each Statement (`For Each...In`) | §5.4.2.4 | `for-each.test.ts` |
| ✅ | P1 | Exit For Statement | §5.4.2.5 | 未テスト |
...
| ✅ | P0 | If Statement（複数行） | §5.4.2.8 | 未テスト |
| ✅ | P0 | Single-line If Statement | §5.4.2.9 | `math-module.test.ts` |
...
| ✅ | P0 | On…GoTo Statement（複数ラベル選択） | §5.4.2.13 | `goto-statement.test.ts` |
| ✅ | P2 | GoSub Statement | §5.4.2.14 | 未テスト |
| ✅ | P2 | Return Statement（GoSub用） | §5.4.2.15 | 未テスト |
| ✅ | P2 | On…GoSub Statement | §5.4.2.16 | 未テスト |
| ✅ | P1 | Exit Sub Statement | §5.4.2.17 | 未テスト |
| ✅ | P1 | Exit Function Statement | §5.4.2.18 | 未テスト |
| ✅ | P1 | Exit Property Statement | §5.4.2.19 | `exit-property.test.ts` |
| ✅ | P2 | RaiseEvent Statement | §5.4.2.20 | 未テスト |
...
| 状態 | 機能 | 仕様書 | テスト |
|------|------|--------|--------|
| ✅ | P0 | Local Variable Declarations (`Dim`) | §5.4.3.1 | 未テスト |
| ✅ | P0 | Local Constant Declarations (`Const`) | §5.4.3.2 | 未テスト |
| ✅ | P1 | ReDim Statement | §5.4.3.3 | 未テスト |
| ✅ | P1 | Erase Statement | §5.4.3.4 | 未テスト |
| ✅ | P1 | Mid Statement（代入形式: `Mid(str, n) = val`） | §5.4.3.5 | `mid-statement.test.ts` |
| ✅ | P2 | MidB / Mid$ / MidB$ Statement | §5.4.3.5 | 未テスト |
| ✅ | P1 | LSet Statement | §5.4.3.6 | `lset-rset.test.ts` |
| ✅ | P1 | RSet Statement | §5.4.3.7 | `lset-rset.test.ts` |
| ✅ | P0 | Let Statement（代入） | §5.4.3.8 | 未テスト |
| ✅ | P0 | Set Statement（オブジェクト参照） | §5.4.3.9 | 未テスト |
...
| ✅ | P2 | Seek Statement | §5.4.5.3 | 未テスト |
| ✅ | P2 | Lock / Unlock Statements | §5.4.5.4/5 | (制限事項: スタブ化) | 未テスト |
| ✅ | P2 | Line Input Statement | §5.4.5.6 | `filesystem.test.ts` |
| ✅ | P2 | Width Statement | §5.4.5.7 | (制限事項: スタブ化) | 未テスト |
...
| ✅ | P2 | Put Statement | §5.4.5.11 | 未テスト |
| ✅ | P2 | Get Statement | §5.4.5.12 | 未テスト |

## 第5章：宣言 (§5.2, §5.3)

| 状態 | 優先度 | 機能 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P0 | Sub / Function 宣言 | §5.3.1.6 | 未テスト |
| ✅ | P0 | Parameter Lists（ByVal, ByRef, Optional, ParamArray） | §5.3.1.5 | 未テスト |
| ✅ | P0 | Property Get / Let / Set 宣言 | §5.3.1.7 | `property.test.ts` |
| ✅ | P2 | Event Handler 宣言 | §5.3.1.8 | 未テスト |
| ✅ | P0 | Variable Declarations（`Dim`） | §5.2.3.1 | 未テスト |
...
| ✅ | P0 | Const Declarations | §5.2.3.2 | 未テスト |
| ✅ | P1 | User Defined Type Declarations（`Type`） | §5.2.3.3 | 未テスト |
...
| ✅ | P1 | Implements Directive | §5.2.4.2 | 未テスト |
| ✅ | P2 | Event Declaration | §5.2.4.3 | 未テスト |
...
| ✅ | P1 | Option Private Module | §5.2.1.4 | 未テスト |
...
| ✅ | P1 | 文字列連結（`&`） | §5.6.9.4 | 未テスト |
| ✅ | P1 | 比較演算子（`=`, `<>`, `<`, `>`, `<=`, `>=`） | §5.6.9.5 | 未テスト |
| ✅ | P1 | Like 演算子 | §5.6.9.6 | (正規表現エスケープおよび[!charlist]対応) | 未テスト |
...
| ✅ | P2 | AddressOf 式 | §5.6.16.8 | `collection-remove.test.ts` |

## 第6章：標準ライブラリ — 変換関数 Conversion Module (§6.1.2.3)

| 状態 | 優先度 | 関数 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P1 | CBool | §6.1.2.3.1.1 | (制限事項: Boolean専用ラッパーを返し、JS側での直接評価には注意が必要) | 未テスト |
...
| ✅ | P1 | CDbl | §6.1.2.3.1.5 | 未テスト |
| ✅ | P2 | CDec | §6.1.2.3.1.6 | `cdec.test.ts` |
| ✅ | P1 | CInt | §6.1.2.3.1.7 | 未テスト |
| ✅ | P1 | CLng | §6.1.2.3.1.8 | 未テスト |
| ✅ | P2 | CLngLng | §6.1.2.3.1.9 | `clnglng.test.ts` |
| ✅ | P2 | CSng | §6.1.2.3.1.11 | `csng.test.ts` |
| ✅ | P1 | CStr | §6.1.2.3.1.12 | 未テスト |
| ✅ | P2 | CVar | §6.1.2.3.1.13 | 未テスト |
...
| ✅ | P1 | LeftB / RightB / MidB | §6.1.2.11.1.x | 未テスト |
...
| ✅ | P0 | InputBox | §6.1.2.8.1.12 | (制限事項: スタブ化。コンソール出力と固定値返却) | 未テスト |
| ✅ | P0 | MsgBox | §6.1.2.8.1.13 | (制限事項: スタブ化。コンソール出力と固定値返却) | 未テスト |
...
| 状態 | 優先度 | 関数 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P0 | UBound | §6（標準ライブラリ） | 未テスト |
| ✅ | P0 | LBound | §6（標準ライブラリ） | 未テスト |
| ✅ | P0 | Array関数 | §6（標準ライブラリ） | 未テスト |
...
| ✅ | P2 | FileAttr | §6.1.2.5.1.4 | (制限事項: スタブ化) | 未テスト |
| ✅ | P2 | FileDateTime | §6.1.2.5.1.5 | 未テスト |
| ✅ | P2 | FileLen | §6.1.2.5.1.6 | 未テスト |
| ✅ | P1 | FreeFile | §6.1.2.5.1.7 | `freefile.test.ts` |
| ✅ | P2 | Loc | §6.1.2.5.1.8 | `filesystem.test.ts` |
| ✅ | P2 | LOF | §6.1.2.5.1.9 | `filesystem.test.ts` |
| ✅ | P2 | Seek | §6.1.2.5.1.10 | `filesystem.test.ts` |
...
| 状態 | 優先度 | 関数 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P2 | DDB | §6.1.2.6.1.1 | 未テスト |
| ✅ | P2 | FV | §6.1.2.6.1.2 | 未テスト |
| ✅ | P2 | IPmt | §6.1.2.6.1.3 | 未テスト |
| ✅ | P2 | IRR | §6.1.2.6.1.4 | 未テスト |
| ✅ | P2 | MIRR | §6.1.2.6.1.5 | 未テスト |
| ✅ | P2 | NPer | §6.1.2.6.1.6 | 未テスト |
| ✅ | P2 | NPV | §6.1.2.6.1.7 | 未テスト |
| ✅ | P2 | Pmt | §6.1.2.6.1.8 | 未テスト |
| ✅ | P2 | PPmt | §6.1.2.6.1.9 | 未テスト |
| ✅ | P2 | PV | §6.1.2.6.1.10 | 未テスト |
| ✅ | P2 | Rate | §6.1.2.6.1.11 | 未テスト |
| ✅ | P2 | SLN | §6.1.2.6.1.12 | 未テスト |
| ✅ | P2 | SYD | §6.1.2.6.1.13 | 未テスト |
...
| 状態 | 優先度 | 機能 | 仕様書 | テスト |
|------|--------|------|--------|--------|
| ✅ | P1 | Clear | §6.1.3.2.1.1 | 未テスト |
| ✅ | P1 | Raise | §6.1.3.2.1.2 | 未テスト |
| ✅ | P1 | Description | §6.1.3.2.2.1 | 未テスト |
| ✅ | P2 | HelpContext | §6.1.3.2.2.2 | 未テスト |
| ✅ | P2 | HelpFile | §6.1.3.2.2.3 | 未テスト |
| ✅ | P2 | LastDllError | §6.1.3.2.2.4 | 未テスト |
| ✅ | P1 | Number | §6.1.3.2.2.5 | 未テスト |
| ✅ | P1 | Source | §6.1.3.2.2.6 | 未テスト |
