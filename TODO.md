...
| ✅ | P1 | 文字列連結（`&`） | §5.6.9.4 |
| ✅ | P1 | 比較演算子（`=`, `<>`, `<`, `>`, `<=`, `>=`） | §5.6.9.5 |
| ✅ | P1 | Like 演算子 | §5.6.9.6 | (正規表現エスケープおよび[!charlist]対応) |
...
| ✅ | P2 | WeekdayName | §6.1.2.4.1.15 |
| ✅ | P2 | MonthName | §6.1.2.4.1.16 |
...
| ✅ | P1 | LeftB / RightB / MidB | §6.1.2.11.1.x |
...
| ✅ | P0 | CreateObject (拡張) | §6.1.2.8.1.4 | (制限事項: Scripting.Dictionary等の主要オブジェクトのスタブ・フェイク実装のみ対応) |
| ✅ | P2 | Environ / Environ$ | §6.1.2.8.1.6 |
| ✅ | P1 | GetObject | §6.1.2.8.1.9 |
| ✅ | P1 | IIf | §6.1.2.8.1.11 |
| ✅ | P0 | InputBox | §6.1.2.8.1.12 | (制限事項: スタブ化。コンソール出力と固定値返却) |
| ✅ | P0 | MsgBox | §6.1.2.8.1.13 | (制限事項: スタブ化。コンソール出力と固定値返却) |
| ✅ | P1 | Shell | §6.1.2.8.1.15 | (制限事項: スタブ化。コマンド実行はせずログ出力のみ) |
