...
- ✅ **循環参照時の `Set = Nothing` 挙動**: 強制クリアと Class_Terminate の呼び出し順 | テスト: `circular-reference-terminate.test.ts`, `circular_reference_terminate_test.vba` (VBA: `CircularTestHelper.cls`, `circular_reference_a_test.cls`, `circular_reference_b_test.cls`, `circular_reference_terminate_test.vba`)
