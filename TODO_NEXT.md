...
- [ ] **デバッグとスタックトレースの精緻化**
  - [ ] 実行時エラー時に、JavaScriptのスタックトレースではなくVBA上のファイル名・行番号・コールスタックを出力する機能 (`Erl` 関数のサポート含む)。

## 7. Webコンパイラ (Web UI) の改善課題
ブラウザ上で完結する実行環境としての完成度を高めます。

- [ ] **仮想ファイルシステム (VFS) の拡充**
  - [ ] `MemoryFileSystem` にワイルドカード対応（例: `Kill "*.txt"`）を追加。
  - [ ] バイナリ操作 (`Get`/`Put`) の `Uint8Array` 互換性の完全検証。
  - [ ] `Dir` 関数によるディレクトリ巡回機能の VFS 対応。
- [ ] **VFS の永続化**
  - [ ] `localStorage` や `IndexedDB` を利用し、リロード後も仮想ファイルが保持されるようにする。
- [ ] **エラーハンドリングの標準化**
  - [ ] すべてのファイル操作関数（`Open`, `Close`, `Print`, `Line Input` 等）に対し、VBA互換のランタイムエラー番号を網羅。
- [ ] **UI/UX の向上**
  - [ ] 仮想ファイルシステム内のファイル一覧を表示・編集できるサイドバー（エクスプローラー）機能。
