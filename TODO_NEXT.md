...
| ファイル/ディレクトリ両対応、テキスト/JSON/outline 出力 | `.vba/.bas/.cls/.frm` を再帰的に処理 |
...
- [x] **識別子の接頭辞クラスタ検出**
  - `COL_*` × 6件 / `CONFIG_*` × 4件 / `ROW_*` × 3件 を検出（TaskScheduler_v1.vba での実証）
...
- [x] **重複ブロック検出（AST N-gram マッチング）**
  ...
  - TaskScheduler_v1.vba での実証: Range読み込み4連パターン・Application設定リストア・グリッド読み込みループなど
