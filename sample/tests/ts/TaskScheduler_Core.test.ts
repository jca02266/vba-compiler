...
async function main() {
    ...
    const calCfg = vbaRunner.run('InitCalendarConfig', []);

    // Test #1: CalcBaseStartIdx
    console.log("\n[Test Suite] CalcBaseStartIdx");

    // Level 1: Always starts at 1, regardless of parent finish
    assert.strictEqual(vbaRunner.run('CalcBaseStartIdx', [1, 10, 1.0]), 1, "Level 1 ignores parent finish");

    // Level 2+ with parent alloc < 0.5: Starts on SAME day
    assert.strictEqual(vbaRunner.run('CalcBaseStartIdx', [2, 10, 0.49]), 10, "Alloc < 0.5 starts same day");
    assert.strictEqual(vbaRunner.run('CalcBaseStartIdx', [3, 5, 0.0]), 5, "Alloc 0 starts same day");

    // Level 2+ with parent alloc >= 0.5: Starts NEXT day
    assert.strictEqual(vbaRunner.run('CalcBaseStartIdx', [2, 10, 0.5]), 11, "Alloc >= 0.5 starts next day");
    assert.strictEqual(vbaRunner.run('CalcBaseStartIdx', [4, 20, 1.0]), 21, "Alloc 1.0 starts next day");
    ...
    assert.strictEqual(vbaRunner.run('GetMaxDailyLoad', ["Charlie", capacityLimits]), 1.0, "Missing user defaults to 1.0");
    assert.strictEqual(vbaRunner.run('GetMaxDailyLoad', ["Alice", capacityLimits]), 0.5, "Known user returns configured capacity");
    assert.strictEqual(vbaRunner.run('GetMaxDailyLoad', ["Bob", capacityLimits]), 0.8, "Known user returns configured capacity");

    // Test #3: CalcDailyAllocation
    console.log("\n[Test Suite] CalcDailyAllocation");

    // Standard Task logic
    assert.strictEqual(vbaRunner.run('CalcDailyAllocation', [1.0, 0.5, false]), 0.5, "Standard: exact fit");
    assert.strictEqual(vbaRunner.run('CalcDailyAllocation', [0.25, 0.5, false]), 0.25, "Standard: limited by capacity");
    assert.strictEqual(vbaRunner.run('CalcDailyAllocation', [1.0, 0.1, false]), 0, "Standard: rounds to 0 if < 0.125 needed");

    // Micro-Task logic
    assert.strictEqual(vbaRunner.run('CalcDailyAllocation', [1.0, 0.1, true]), 0.1, "Micro: capacity 1.0 > 0.1, gets 0.1");
    assert.strictEqual(vbaRunner.run('CalcDailyAllocation', [0.05, 0.1, true]), 0, "Micro: capacity < 0.1, skips");
    ...
    vbaRunner.run('UpdateLevelFinish', [1, 15, 0.8, levelFinish, levelAlloc]);
    ...
    vbaRunner.run('UpdateLevelFinish', [1, 15, 0.1, levelFinish, levelAlloc]);
    ...
    vbaRunner.run('UpdateLevelFinish', [1, 15, 1.0, levelFinish, levelAlloc]);
    ...
    const resultDict = vbaRunner.run('BuildCapacityDict', [mockConfigData]);
    ...
    vbaRunner.run('ScanLockedRows', [taskCfg, 3, 3, mockMetaData, mockGridData, assigneeUsage]);
    ...
    vbaRunner.run('ClearTaskGridRow', [2, 3, gridToClear]);
    ...
    const resRow1 = vbaRunner.run('TestFindLockedTaskFinish', [1, 4, mockLockedGrid]);
    assert.strictEqual(resRow1, "2|1", "Row 1 Finish Idx 2, Alloc 1.0");

    const resRow2 = vbaRunner.run('TestFindLockedTaskFinish', [2, 4, mockLockedGrid]);
    ...
    const resRow1Schedule = vbaRunner.run('TestScheduleUnlockedTask', [taskCfg, calCfg, 1, 5, 1, mockMetaSchedule, mockHolidayData, capacityLimits, mockGridSchedule, mockPersonUsage]);
    ...
    const resRow2Schedule = vbaRunner.run('TestScheduleUnlockedTask', [taskCfg, calCfg, 2, 5, 3, mockMetaSchedule, mockHolidayData, capacityLimits, mockGridSchedule, mockPersonUsage]);
    ...
    assert.strictEqual(vbaRunner.run('CalcNumRows', [19, taskCfg]), 1, "lastRow=19 -> 1 row");
    assert.strictEqual(vbaRunner.run('CalcNumRows', [25, taskCfg]), 7, "lastRow=25 -> 7 rows");
    assert.strictEqual(vbaRunner.run('CalcNumRows', [18, taskCfg]), 0, "lastRow=18 -> 0 rows (no data)");

    assert.strictEqual(vbaRunner.run('CalcNumDays', [24, calCfg]), 1, "lastCol=24 -> 1 day");
    assert.strictEqual(vbaRunner.run('CalcNumDays', [30, calCfg]), 7, "lastCol=30 -> 7 days");
    assert.strictEqual(vbaRunner.run('CalcNumDays', [23, calCfg]), 0, "lastCol=23 -> 0 days (no data)");
    ...
    assert.strictEqual(vbaRunner.run('GetTaskLevel', [mockMetaRow, 2, taskCfg]), 0, "Row 2 empty level = 0");
